...
import pygame
# Zur Darstellung nutze ich der Einfachheit halber pygame.
# Zunächst pygame initalisiert. Dies ist nur zur Visualisierung.
pygame.init()
screen_width, screen_height = 1500, 925
...
def get_color(distance, max_distance):
    '''Diese Fuktion gibt eine Farbe im RGB-Spektrum zurück, um die 3D-Visualisierung zu vereinfachen.
       Die Farbe skaliert auf von Blau zu Rot, wobei die länge der Skala durch max_distance gegeben wird.
    '''
    ratio = min(abs(distance / max_distance), 1.0)
...
def blit_screen(punktwolke, clustering):
    screen.fill(BLACK)

    # Darstellung der Punkte und Objekte
    draw_isometric_view(clustering, (screen_width*8/10 ,screen_height*7/10))

    screen.blit(font.render(f"Scale = {scale} (+/-)", True, WHITE), (10, 10))
    screen.blit(font.render(f"Eps = {eps} (./,)", True, WHITE), (10, 10 + font_height))
    screen.blit(font.render(f"p -> paused", True, WHITE), (10, 10 + 2 * font_height))
    screen.blit(font.render(f"Messungen: {mess_index}", True, WHITE), (10, 10 + 3 * font_height))

    match warnlicht:
        case 0:
            pygame.draw.circle(screen, (0, 255, 0), (screen_width-50, 50), 40)
        case 1:
            pygame.draw.circle(screen, (255, 255, 0), (screen_width - 50, 50), 40)
        case 2:
            pygame.draw.circle(screen, (255, 0, 0), (screen_width - 50, 50), 40)

def draw_movement_vector(vector):
    v_normed = vector / np.linalg.norm(vector)
    x, y, z = v_normed
    pygame.draw.line(screen, WHITE, (100, 100), (100+y*50, 100-z*50))
    pygame.draw.line(screen, WHITE, (100, 200), (100-x*50, 200-z*50))
    pygame.draw.line(screen, WHITE, (100, 300), (100-y*50, 300-x*50))

def draw_isometric_view(clustering, center):
    x_shift, y_shift = center

    def get_screen_koo(x, y, z):
        x_koo = x_shift + scale * (0.85 * x - 0.85 * y)
        y_koo = y_shift + scale * (-0.3 * x - 0.3 * y - z)
        return x_koo, y_koo

    # Nullpunkt zeichnen
    pygame.draw.line(screen, (255, 0, 0), (x_shift, y_shift), get_screen_koo(50, 0, 0))
    pygame.draw.line(screen, (0, 255, 0), (x_shift, y_shift), get_screen_koo(0, 50, 0))
    pygame.draw.line(screen, (0, 0, 255), (x_shift, y_shift), get_screen_koo(0, 0, 50))

    # Weisse Punkte als Position der Sensoren:
    for position in sensor_positions:
        x, y, z = position
        screen_koo = get_screen_koo(x, y, z)
        pygame.draw.circle(screen, (255, 255, 255), screen_koo, 5)

    labels = clustering.labels_

    len_groupings = len(all_groupings) + 1
    for i, group in enumerate(all_groupings):
        for point in group.grouped_points:
            x, y, z = point
            screen_koo = get_screen_koo(x, y, z)
            color = get_color(i+1, len_groupings)
            pygame.draw.circle(screen, color, screen_koo, 5)

    return # Entferne dieses return falls alle Datenpuntke dargestellt werden sollen!

    # Darstellung der Punktwolke:
    for i, point in enumerate(punktwolke):
        x, y, z = point
        screen_koo = get_screen_koo(x, y, z)
        #color = get_color(z + 50, 500)
        color = get_color(labels[i]+1, n_clusters+1)
        pygame.draw.circle(screen, color, screen_koo, 5)

    # Darstellung der Boxen um die erkannten Objekte:
    n_labels = len(set(labels)) - (1 if -1 in labels else 0)
    np_punktwolke = np.array(punktwolke)
    for label in set(labels):
        if label != -1:
            cluster_points = np_punktwolke[labels == label]
            color = get_color(label + 1, n_labels + 1)
            draw_bounding_box(cluster_points, color, screen)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_p:
                ...
                while paused:
                    for event in pygame.event.get():
                        if event.type == pygame.KEYUP: paused = False
            elif event.key == pygame.K_PLUS:   scale += 0.1
            elif event.key == pygame.K_MINUS:  scale -= 0.1
            elif event.key == pygame.K_COMMA:  eps -= 10
            elif event.key == pygame.K_PERIOD: eps += 10
    ...
    blit_screen(punktwolke, clustering)
    # Die Momentange Punktwolke wird dargestellt.
    # Damit das clustering nicht mehrfach berechnet werden muss wird es übergeben.
    ...
    for group in all_groupings:
        ...
        if np.linalg.norm(movement_vector) > movement_vector_sensibility:
            ...
            print(f"Bewegung erkannt: {movement_vector}")
            # Erkannt Bewegungsvektoren können ausgegeben werden.

            warnlicht = check_for_collision(group, movement_vector)
            # Das Warnlicht wird über die check_for_collision-Methode eigestellt.

            #draw_movement_vector(movement_vector)
            # Hier wird nochmal
