...
IMAGE_TIMER = 0.5
# Defines the time between showing an output-image in seconds.
...
def read_json(json_input, time_now):
    ...
    for sensor_id in range(SENSORS_ON_RING):
        ...
        if sensor_name not in json_data:
            print(f"Warnung: {sensor_name} nicht in JSON-Daten gefunden.")
            continue
...
# Open3D-Implementierung
if DO_VISUALIZATION:
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name="Open3D Live Voxel", width=800, height=600)

    # Empty PointCloud-Objekt
    pcd = o3d.geometry.PointCloud()
    vis.add_geometry(pcd)  # Füge die leere Geometrie hinzu

    # Sensor = red static Kugel
    sensor_coords_scaled = sensor_positions / VOXEL_GRID_SIZE
    for i, pos in enumerate(sensor_coords_scaled):
        sensor_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.08)
        sensor_sphere.paint_uniform_color([1.0, 0.0, 0.0])
        sensor_sphere.translate(pos)
        vis.add_geometry(sensor_sphere)

    ctr = vis.get_view_control()
    ctr.set_lookat([VOXEL_MATRIX_SHAPE[0] / 2, VOXEL_MATRIX_SHAPE[1] / 2, VOXEL_MATRIX_SHAPE[2] / 2])
    ctr.set_zoom(4)
    ...
    if active_voxels.size > 0 and time_last_cycle > 0:
        ...
        for detected_object in all_detected_objects:
            ...
            if mean_displacement.any():
                # Berechne die vergangene Zeit seit dem letzten Zyklus
                time_delta = time_now - time_last_cycle_timestamp if 'time_last_cycle_timestamp' in locals() else 0.01
                if time_delta == 0: time_delta = 0.01  # Division durch Null verhindern
                ...
                if total_velociy > TOTAL_VELOCITY_TOLERANCE:
                    print(
                        f"t = {(time_now - start_time):.2f}s: Movement of Object detected: v={velocity_vector}mm/s ({total_velociy}mm/s)")

            detected_object.clean(time_now)

    if DO_VISUALIZATION: #next_image_timer < time_now and
        ...
        if active_voxels.size > 0:
            ...
            for i, obj in enumerate(all_detected_objects):
                if obj.listed_voxels_list:  # Prüfen ob Voxel vorhanden sind
                    # Letzte Voxel-Position des Objekts holen
                    obj_voxels = obj.listed_voxels_list[-1]
