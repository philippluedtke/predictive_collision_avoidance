...
Real-life testing revealed many coding and design-flaws of our approach. Although the program handled the increased data given by 6 additional sensor quite good with cycle-times of 85 to 65ms, the main concern were significant problems with the detection and especially the distinction of the different objects. We tried to adress these problems by adjusting the parameters, particualry the epsilon of the DBScan (meaning the distance at which two voxels are merged). A major cause for errors were the static surroundings, which often grouped together with nearby moving objects or merged and then unmerged with other static objects, both resulting in motion errors. We could only partially adress this by adjusting the DBScan parameters, so we decided to introduce a significant adjustment by scanning for static objects before starting the main program as part of a preperation function. We then assume that these static voxels are not relevant for the object-detection. This way, we were able to reduce the merging errors considerably, but the process we use to identify the static objects is still not certain or fully developed. 

A other problem was the cycle-clock-speed. We tried classic boolean-based voxels as well as the time-based approach. The main advatages of the boolean method are a theoretically better performance, although we could not demostrate this, and the possibility of better movement-reconstruction, meaning the ability to trace back the movement further back in time as it would be possible using the time based method as each timeframe is seperately recorded. The advatage of the time based method are a more consistent tracking, more adjustability in form of the TIME_TOLERANCE variable and the enhanced expandability for possible other sensor rings as there are no time-frames but a more fluid memorization of the recieved sensor data.
...
This video shows our test with two moving objects. The beginning (0-2 seconds) shows one person apporaching the sensors relatively fast, resulting in erratic behavior. Until 5 seconds the movement can then be identified quite well. At 6 seconds, the second person apporaches the sensors. While both persons are then near the sensors, the detected objects merge and result in no (or very small) detected motion. Around 11 seconds, the first person exits the sensor-range and both objects seperate again. The second persons movement can then be observer well again unit he exits the sensor range as well.
